...
select * from students where age > 18 and gender = 2;
		...
		-- or
		-- 18以上或者身高高过180(包含)以上
select * from where age > 18 or height >= 180;
...
-- 连接查询
	...
	-- 按照要求显示姓名、班级
select students.name,classes.name from students inner join on classes students.cls_id = classes.id;
...
select c.name ,s.* from students as s inner join classes as c on s.cls_id = c.id;
	...
	select c.name, s.* from students inner join classes as c on s.cls_id = c.id order by c.name asc;

	
	
	-- 当时同一个班级的时候，按照学生的id进行从小到大排序
select c.name, s.* from students inner join classes as c on s.cls_id = c.id order by c.id asc, s.id asc;	



	-- left join
	-- 查询每位学生对应的班级信息
select s.name,c.name from students as s left into classes as c on s.cls_id = c.id;	


	-- 查询没有对应班级信息的学生
select s.*,c.name from students as s left into classes as c on s.cls_id = c.id where c.name is null;
