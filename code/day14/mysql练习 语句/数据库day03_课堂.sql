...
-- 2 关联查询
select goods.* from goods inner join (select cate_name,max(price) as price  from goods group by cate_name) as max_goods
 on goods.cate_name = max_goods.cate_name and goods.price = max_goods.price;
