...
select name, price from cate_name = '超级本';
...
select cate_name, max_price from goods group by cate_name;


-- 2 关联查询
select * from goods as a inner join (select cate_name,max_price as price from goods group by cate_name) on a.cate_name = a2.cate_name and a.price = a2.price;
...
--2. 通过特殊的update 进行更新
update (goods left join goods_cates on goods.cate_name = goods_cates.name) set goods.cate_name = goods_cates.id; 

--第六 更新品牌信息成品牌信息表的id
--1. 把要更新的表两张表进行内联
select * from goods left join goods_brands on goods.brand_name = goods_brands.name;


--2.通过特殊的update语句进行更新 
update (goods left join goods_brands on goods.brand_name = goods_brands.name) set goods.brand_name = goods_brands.id;
